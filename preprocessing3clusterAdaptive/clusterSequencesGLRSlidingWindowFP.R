...
for (seq in 1:4){
  print("sequence")
  print(seq)
  # HMM fitting repeated from same seed as in step 1 preprocessing
  ...
  adaptiveNewClusterThreshold = 2 * splittingPointsThresholds[seq]
  ...
  newClusterNoMergeItersRemaining = 0
  ...
  while(analysisWindowEndIndex < endSequenceIndex){
    ...
    if (testGLR > adaptiveProbeChangeThreshold){
      ...
      testNewHyperParamsAll <- lib$bayes$updateHyperparamsNormalGamma(testEndSuffStats, clusterHyperParams[[closestClusterAll]])
      testNewHyperParamsPre <- lib$bayes$updateHyperparamsNormalGamma(testEndSuffStats, clusterHyperParams[[closestClusterPre]])
      testGLRMergePre <- GLRposterior(clusterSuffStats[[closestClusterPre]], testEndSuffStats, 
                                      clusterHyperParams[[closestClusterPre]], testNewHyperParamsPre)
      testGLRMergeAll <- testGLRMergePre
      if (closestClusterPre != closestClusterAll){
        testGLRMergeAll <- GLRposterior(clusterSuffStats[[closestClusterAll]], testEndSuffStats, 
                                        clusterHyperParams[[closestClusterAll]], testNewHyperParamsAll)
        
      }
      # update the previous cluster
      clusterSuffStats[[currentClusterIndex]] <- splitList[[4]]
      clusterHyperParams[[currentClusterIndex]] <- splitList[[2]]
      clusterPosteriorStudentParams[[currentClusterIndex]] <- 
        lib$bayes$studentParams(clusterHyperParams[[currentClusterIndex]])
      if (testGLRMergeAll > adaptiveNewClusterThreshold){
        # create new cluster
        print("new cluster")
        print(splittingInd)
        currentClusterIndex <- length(clusterSuffStats) + 1
        clusterSuffStats[[currentClusterIndex]] <- testEndSuffStats
        clusterHyperParams[[currentClusterIndex]] <- 
          lib$bayes$updateHyperparamsNormalGamma(testEndSuffStats, initHyperParams)
        clusterPosteriorStudentParams[[currentClusterIndex]] <- 
          lib$bayes$studentParams(clusterHyperParams[[currentClusterIndex]])
        ...
        changedCluster = TRUE
        newClusterNoMergeItersRemaining = newClusterNoMergeItersInit - splitList[[1]]
      } 
      else if ((testGLRMergePre < adaptiveMergingThreshold) & (closestClusterPre != currentClusterIndex)){ 
        # change current cluster
        currentClusterIndex <- closestClusterPre
        # add splitting point
        splittingPoints[[length(splittingPoints)+1]] <- splittingInd
        clusterIndices[[length(splittingPoints)+1]] <- currentClusterIndex
        clusterSuffStats[[currentClusterIndex]] <- splitList[[5]]
        clusterHyperParams[[currentClusterIndex]] <- splitList[[3]] 
        clusterPosteriorStudentParams[[currentClusterIndex]] <- 
          lib$bayes$studentParams(clusterHyperParams[[currentClusterIndex]])
        adaptiveStudentParamsList <- c(adaptiveStudentParamsList, clusterPosteriorStudentParams[[currentClusterIndex]])
        adaptiveStudentParamsChanges <- c(adaptiveStudentParamsChanges, splittingInd)
        changedCluster = TRUE
      } else if (closestClusterAll != currentClusterIndex){
        # change current cluster
        currentClusterIndex <- closestClusterAll
        # add splitting point
        splittingPoints[[length(splittingPoints)+1]] <- splittingInd
        clusterIndices[[length(splittingPoints)+1]] <- currentClusterIndex
        clusterSuffStats[[currentClusterIndex]] <- splitList[[5]]
    ...
    if (changedCluster){
      ...
      clusterSuffStats[[currentClusterIndex]] <- Map("+", clusterSuffStats[[currentClusterIndex]], 
                                                       removeSuffStatsStep)
      clusterHyperParams[[currentClusterIndex]] <- 
          lib$bayes$updateHyperparamsNormalGamma(removeSuffStatsStep, clusterHyperParams[[currentClusterIndex]])
      clusterPosteriorStudentParams[[currentClusterIndex]] <- 
          lib$bayes$studentParams(clusterHyperParams[[currentClusterIndex]])
      adaptiveStudentParamsList <- c(adaptiveStudentParamsList, clusterPosteriorStudentParams[[currentClusterIndex]])
      adaptiveStudentParamsChanges <- c(adaptiveStudentParamsChanges, analysisWindowStartIndex)
      
      # check if the cluster should be merged
      if (newClusterNoMergeItersRemaining > 0){
        newClusterNoMergeItersRemaining = newClusterNoMergeItersRemaining -1
      }
      else {
        closestClustersPreAllToCurrent = 
          closestClustersGLRNormalGammaPreAllFromInd(
            clusterHyperParams, clusterSuffStats, 
            currentClusterIndex, nPreprocessedClusters)
        closestClusterGLRPreprocessing = closestClustersPreAllToCurrent[[2]]
        closestClusterGLRAll = closestClustersPreAllToCurrent[[4]]
        lowInd = closestClustersPreAllToCurrent[[1]]
        highInd = currentClusterIndex
        mergeClusters = FALSE
        # prefer merging new clusters with preprocessing clusters
        if ((closestClusterGLRPreprocessing < clusterMergingThresholdPre) & 
            (currentClusterIndex > nPreprocessedClusters)) {
          # merge the two clusters into the one with lowest index
          if (currentClusterIndex < lowInd){
            lowInd = currentClusterIndex
            highInd = closestClustersPreAllToCurrent[[1]]
          }
          mergeClusters = TRUE
        }
        else if(closestClusterGLRAll < clusterMergingThresholdAll){
          lowInd = closestClustersPreAllToCurrent[[3]]
          highInd = currentClusterIndex
          if (currentClusterIndex < lowInd){
            lowInd = currentClusterIndex
            highInd = closestClustersPreAllToCurrent[[3]]
          }
          mergeClusters = TRUE
        }
        if (mergeClusters){
          print(analysisWindowStartIndex)
          print("merging ")
          print(lowInd)
          print("with")
          print(highInd)
          clusterHyperParams[[lowInd]] <- 
            lib$bayes$updateHyperparamsNormalGamma(clusterSuffStats[[highInd]], 
                                                            clusterHyperParams[[lowInd]])
          clusterSuffStats[[lowInd]] <- Map("+", clusterSuffStats[[lowInd]], clusterSuffStats[[highInd]])
          clusterPosteriorStudentParams[[lowInd]] <- 
            lib$bayes$studentParams(clusterHyperParams[[lowInd]])
          # remove the highest
          clusterHyperParams <- clusterHyperParams[-highInd]
          clusterSuffStats <- clusterSuffStats[-highInd]
          clusterPosteriorStudentParams <- clusterPosteriorStudentParams[-highInd]
          currentClusterIndex = lowInd
          if (highInd <= nPreprocessedClusters){
            # merged two preprocessing clusters
            nPreprocessedClusters = nPreprocessedClusters - 1
          }
          for(i in 1:length(clusterIndices)){
            if (clusterIndices[[i]] == highInd){
              clusterIndices[[i]] = lowInd
            }
            if (clusterIndices[[i]] > highInd){
  ...
  write.table(as.data.frame(adaptiveStudentParamsList, stringsAsFactors = FALSE),
              file=filename, quote=F,sep=",",row.names=F, col.names=F)
  
  filename = paste(outputDir, "adaptiveStudentParamsChangeIndices", seq, ".csv", sep="")
  write.table(as.data.frame(adaptiveStudentParamsChanges, stringsAsFactors = FALSE),
              file=filename, quote=F,sep=",",row.names=F, col.names=F)
